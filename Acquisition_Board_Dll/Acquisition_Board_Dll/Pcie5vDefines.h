...
#define ADC_BLOCKSIZE           (32*1024*1024)	
...
#define SEL_WRITE_WHOLE_ROW     false	// true for dual 8-bit A/D only boards? or always false?	
...
#define DRAM_DQS_IDELAY_VAL     0       // 0 @ 300MHz
...
#define MAX_DQS_DELAY           1   // 1   // 64 max,
#define MIN_DQ_DELAY            5   // 8   // higher than 18 has lots of errors, 7-14 seem to be the best
#define MAX_DQ_DELAY            12   // 9  // 64 max, 
...
// All the PIO registers should be accesable by user program. But they should be declared in the DLL so the user doesn't do any bit manipulation.
typedef struct
{
    unsigned long CaptureAdjReg;        // 0x06 PIO_OFFSET_CAPTUREADJ
    unsigned long DRAMAdjReg;           // 0x0D PIO_OFFSET_DRAM_ADJUST
    unsigned long AdcDataSetup;         // 0x10 PIO_OFFSET_ADC_DATA_SETUP
